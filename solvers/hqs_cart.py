...
weight_dir = Path(__file__).parent.parent / "weights" /"drunet2d"/"model_weights"
...
class Denoiser(torch.nn.Module):
    ...
    def forward(self, x, sigma, norm=True):
        x = torch.permute(torch.view_as_real(x.squeeze(0)), (2, 0, 1)).to("cuda")[None]
        if norm:
            x = x / 100
        x_ = torch.permute(self.denoiser(x, sigma).to("cpu"), (0, 2, 3, 1))
        if norm:
            x_ = x_ * 100
        return torch.view_as_complex(x_.contiguous())[0]
